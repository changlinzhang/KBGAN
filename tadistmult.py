...
class TADistMultModule(BaseModule):
    def __init__(self, n_ent, n_rel, config):
        super(TADistMultModule, self).__init__()
        sigma = 0.2
...
class TADistMult(BaseModel):
    def __init__(self, n_ent, n_rel, config):
        super(TADistMult, self).__init__()
        self.mdl = TADistMultModule(n_ent, n_rel, config)
        self.mdl.cuda()
        self.config = config
